  ...
  for (contrast_idx in seq_along(contrasts)) {
    ...
    for (i in seq_along(group_ids)) {
      ...
      if (aggregate_by == "domain_name") {
        ...
        protein_name <- paste0(n_distinct(group_peptides$ProteinName), " proteins")
        domain_id <- paste0(n_distinct(group_peptides$DomainID), " instances")
      ...
      make_result <- function(p_right, p_left, method) {
        res <- data.frame(
          DomainID = domain_id,
