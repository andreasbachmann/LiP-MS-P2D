...
deduplicate_results <- function(results, global_mode = FALSE) {
  if (global_mode) {
    # global mode
    deduped <- results %>%
      group_by(Label, mean_log2FC, median_log2FC, adj_p_right, adj_p_left, n_peptides, n_peptides_used) %>%
    ...
    # instance mode
    deduped <- results %>%
      group_by(ProteinName, Label, mean_log2FC, median_log2FC, adj_p_right, adj_p_left, n_peptides, n_peptides_used) %>%
      slice_min(nchar(DomainName), n = 1, with_ties = FALSE) %>%
      ungroup() %>%
      select(
